					...
					<div className={style.container}>
						...
						<form onSubmit={handleSubmit(handleInfos)} className={style.form}>
							...
							<label className={`${style.label} ${!errorZipCode ? style.labelNormal : style.labelError}`}>
								...
								})} placeholder="CEP" className={style.input} onFocus={() => errorZipCode ? setErrorZipCode(false) : ''} onBlur={checkZip} maxlength={8} minLength={8} inputMode="numeric" />
							</label>
							<label className={`${style.label} ${btn ? style.labelNormal : style.labelError}`}>
								...
								<input {...register("endereço", {
									required: " ",
								})} inputMode="text" placeholder="Endereço" className={style.input} maxlength={32} />
							</label>
							<label className={`${style.label} ${btn ? style.labelNormal : style.labelError}`}>
								<FaSearchLocation className={`${style.icon} ${btn ? style.iconNormal : style.iconError}`} />
								<input {...register("bairro", {
									required: " ",
								})} inputMode="text" placeholder="Bairro" className={style.input} maxlength={32} />
							</label>
							<label className={`${style.label} ${btn ? style.labelNormal : style.labelError}`}>
								<MdOutlineLocationCity className={`${style.icon} ${btn ? style.iconNormal : style.iconError}`} />
								<input {...register("complemento", {
								})} inputMode="text" placeholder="Complemento" className={style.input} maxlength={100} />
							</label>
							<label className={`${style.label} ${btn ? style.labelNormal : style.labelError}`}>
								<BsPhoneFill className={`${style.icon} ${btn ? style.iconNormal : style.iconError}`} />
								<Input {...register("telefone", {
									required: " ",
								})} inputMode="tel" defaultCountry="BR" value={mobile} onChange={setMobile} placeholder="Telefone" className={style.input} maxlength={15} />
							</label>
							{btn ?
